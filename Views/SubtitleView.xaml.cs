...
using FFmpegApi.Subtitles;
using FFmpegApi.Views.Enums;
...
/// </summary>
public partial class Subtitles : UserControl, INotifyPropertyChanged
{
    /// <summary>
    ...
    /// </summary>
    private string _targetLanguage = "pl"; // Default target language (e.g., English)
    ...
    /// <summary>
    /// Represents the video player instance used to manage video playback operations.
    /// </summary>
    /// <remarks>This field should be initialized before use to ensure that video playback functions
    /// correctly. It is intended for internal use within the class to coordinate playback state and control.</remarks>
    private VideoLectore _lectore;
    ...
    public void StartLectore()
    {
        ...
        if (_lectoreOnOff)
        {
            _lectore = new VideoLectore("Microsoft Paulina Desktop");
        }
    }
    ...
    private async Task<string> TranslateWithAiAsync(string text)
    {
        ...
        {
            Logger.Info("OpenAI API key is not set. Skipping translation.");
            return text;
        }
        ...
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                {
                    Logger.Error($"AI Translation failed: {response.StatusCode} - {response.ReasonPhrase}");
                    return text;
                }
            }
        }
        ...
        {
            Logger.Error($"AI Translation exception: {ex.Message}");
            return text;
        }
        finally
        {
    ...
    //    {
    //        Logger.Info("Gemini API key is not set. Skipping translation.");
    //        return text;
    //    }
    ...
    //            if (!response.IsSuccessStatusCode)
    //            {
    //                Logger.Info($"Gemini Translation failed: {response.StatusCode} - {response.ReasonPhrase}");
    //                return text;
    //            }
    ...
    //    catch (Exception ex)
    //    {
    //        Logger.Info($"Gemini Translation exception: {ex.Message}");
    //        return text;
    //    }
    //    finally
    //    {
